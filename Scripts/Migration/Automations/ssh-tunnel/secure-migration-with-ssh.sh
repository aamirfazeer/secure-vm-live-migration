...
setup_ssh_tunnel() {
    ...
    sshpass -p "$SOURCE_PASS" ssh -o StrictHostKeyChecking=no root@$SOURCE_IP \
        "sshpass -p '$DEST_PASS' ssh -f -N -L ${TUNNEL_PORT}:localhost:${TUNNEL_PORT} root@${DESTINATION_IP} -o StrictHostKeyChecking=no ServerAliveInterval=30" > /dev/null 2>&1
...
# ----------------- Starting the Workload - CLEAN VERSION -------------------
start_workload() {
    ...
    sshpass -p "$VM_PASS" ssh -o StrictHostKeyChecking=no root@$VM_IP \
        ...
        timeout 300 ./quicksort" > "logs/${LOG_FOLDER}/${LOG_ID}_quick_sort.txt" 2>&1 &
...
trigger_secure_migration() {
    echo ">>> Triggering Secure Migration via SSH Tunnel"
    ...
    if [ "$TYPE" = "precopy" ]; then
        ...
        echo ">>> ERROR: Unknown migration type: $TYPE"
        return 1
    fi
}
...
for (( i=1; i<=$ITERATIONS; i++ )); do
    echo "========================================="
    echo ">>> Starting Secure Migration Iteration $i"
    echo "========================================="

    timestamp=$(date "+%Y%m%d_%H%M%S")
    LOG_ID="${TYPE}_ssh_${VM}_${SIZE}_${timestamp}"
    ...
    if ! setup_ssh_tunnel; then
        echo ">>> ERROR: Failed to setup SSH tunnel, skipping iteration $i"
        continue
    fi

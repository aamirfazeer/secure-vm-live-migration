...
void *timer_thread(void *arg) {
    int seconds = *((int *)arg);
    sleep(seconds);
    printf("Time limit of %d seconds reached. Terminating process.\n", seconds);
    exit(0);
    return NULL; // This line won't actually be reached
}
...
int main(int argc, char ** argv){
    ...
    // Create timer thread
    pthread_t tid;
    if (pthread_create(&tid, NULL, timer_thread, &time_limit) != 0) {

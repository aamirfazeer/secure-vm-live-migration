...
# ══════════════════════════════════════════════════════════════════════
#  ycsb_ssh.sh  —  YCSB Workload VM Migration (SSH Tunnel)
# ══════════════════════════════════════════════════════════════════════
...
# ─── SSH Tunnel helpers ───────────────────────────────────────────────
setup_ssh_tunnel() {
    echo ">>> Setting up SSH tunnel (port $TUNNEL_PORT)..."
    ...
    sshpass -p "primedirective" ssh -o StrictHostKeyChecking=no root@"$SOURCE_IP" \
        "sshpass -p 'primedirective' ssh -f -N \
         -L ${TUNNEL_PORT}:localhost:${TUNNEL_PORT} root@${DESTINATION_IP} \
         -o StrictHostKeyChecking=no -o ServerAliveInterval=30" > /dev/null 2>&1
    sleep 10

    local TUNNEL_PID
    TUNNEL_PID=$(sshpass -p "primedirective" ssh -o StrictHostKeyChecking=no root@"$SOURCE_IP" \
        "lsof -ti :$TUNNEL_PORT -sTCP:LISTEN" 2>/dev/null)
...
run_iteration() {
    ...
    echo ""
    echo "=========================================="
    echo ">>> [SSH Tunnel] $CURRENT_TYPE — Iteration $ITER"
    echo "=========================================="

    timestamp=$(date "+%Y%m%d_%H%M%S")
    LOG_ID="${CURRENT_TYPE}_ssh_${VM_IMG}_${RAM_SIZE}_${timestamp}"
    ...
    if ! setup_ssh_tunnel; then
        echo "❌ Skipping iteration $ITER — tunnel setup failed"
        return 1
    fi

    # 3. Wait for utility VM
    ...
    # 5. Trigger migration via SSH tunnel
    bash "$COMMON_SCRIPTS/trigger_migration.sh" \
        --source="$SOURCE_IP" --type="$CURRENT_TYPE" \
        --mode="ssh" --tunnel_port="$TUNNEL_PORT"
...
# ─── Main ────────────────────────────────────────────────────────────
echo "══════════════════════════════════════════════"
echo "  YCSB SSH Tunnel Migration  |  $(date)"
...
for CURRENT_TYPE in "${MIGRATION_TYPES[@]}"; do
    echo ""
    echo "######################################################"
    echo "#  Starting SSH $CURRENT_TYPE  ($ITERATIONS iterations)"
    echo "######################################################"
...
echo ""
echo "══════════════════════════════════════════════"
echo "  SSH YCSB Complete  |  Logs: logs/$LOG_FOLDER"
echo "══════════════════════════════════════════════"

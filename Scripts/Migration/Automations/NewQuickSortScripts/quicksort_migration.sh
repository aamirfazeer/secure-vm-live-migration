...
collect_quicksort_logs() {
    local log_id=$1
    echo ">>> Collecting QuickSort logs from VM"
    
    # Copy QuickSort log from VM to local logs folder
    sshpass -p "$VM_PASSWORD" scp -o StrictHostKeyChecking=no \
        root@$VM_IP:/tmp/quicksort_${log_id}.log \
        "$LOG_FOLDER/${log_id}_quicksort.txt" 2>/dev/null
    
    # Check if QuickSort is still running
    QUICKSORT_CHECK=$(sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP "ps -ef | grep quicksort | grep -v grep" 2>/dev/null)
    if [[ -n "$QUICKSORT_CHECK" ]]; then
        echo "✅ QuickSort is still running on destination VM"
    else
        echo "⚠️  QuickSort has completed or stopped"
    fi
}

stop_quicksort_workload() {
    echo ">>> Stopping QuickSort workload"
    sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP \
        "pkill -f quicksort" 2>/dev/null
}

start_quicksort_workload() {
    local log_id=$1
    echo ">>> Starting QuickSort workload as background process"
    
    # Create a unique log file name inside VM
    local vm_log_file="/tmp/quicksort_${log_id}.log"
    
    # Start QuickSort with proper nohup and disown to survive SSH disconnect
    sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP << EOF
cd /home/vmuser/Desktop
nohup ./quicksort > ${vm_log_file} 2>&1 < /dev/null &
disown
EOF
    
    sleep 3
    
    # Verify QuickSort is running
    QUICKSORT_CHECK=$(sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP "ps -ef | grep -v grep | grep quicksort")
    if [[ -n "$QUICKSORT_CHECK" ]]; then
        echo "✅ QuickSort is running on VM"
        echo "$QUICKSORT_CHECK"
    else
        echo "❌ Warning: QuickSort is NOT running!"
        echo ">>> Trying to check what happened..."
        sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP "ls -l /home/vmuser/Desktop/quicksort"
    fi
}
...
for (( i=1; i<=$ITERATIONS; i++ )); do
    ...
    if wait_for_migration_completion "$LOG_ID"; then
        ...
        # Verify QuickSort is still running after migration
        echo ">>> Verifying QuickSort after migration..."
        sleep 5
        QUICKSORT_POST_MIGRATION=$(sshpass -p "$VM_PASSWORD" ssh -o StrictHostKeyChecking=no root@$VM_IP \
            "ps -ef | grep -v grep | grep quicksort" 2>/dev/null)
        
        if [[ -n "$QUICKSORT_POST_MIGRATION" ]]; then
            echo "✅ QuickSort is RUNNING on destination after migration!"
            echo "$QUICKSORT_POST_MIGRATION"
        else
            echo "❌ QuickSort is NOT running on destination after migration!"
        fi
    else
    ...
    # Wait 20s after migration completes before collecting logs
    echo ">>> Waiting 20 seconds after migration completion before cleanup..."
    sleep 20
    
    # Collect QuickSort logs
    collect_quicksort_logs "$LOG_ID"

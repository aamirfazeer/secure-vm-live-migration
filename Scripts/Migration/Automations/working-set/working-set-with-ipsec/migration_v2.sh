...
# Usage: ./migration_script.sh [options]
...
for ARG in "$@"; do
  ...
  if ! $MATCHED; then
    ...
    echo "VM sizes: 8192, 12288, 16384, 20480, all (in MB)"
    exit 1
  fi
done
...
echo -e "${BLUE}========================================================${NC}"
echo -e "${BLUE}=== VM Migration Test with Working Set ===${NC}"
echo -e "${BLUE}========================================================${NC}"
if [ "$MIGRATION_TYPE" = "all" ]; then
...
for CURRENT_SIZE in "${VM_SIZES[@]}"; do
    ...
    for MIGRATION_TYPE_RUN in "${MIGRATION_TYPES[@]}"; do
        echo -e "${YELLOW}=== Starting $MIGRATION_TYPE_RUN Migration Tests (${CURRENT_SIZE}MB) ===${NC}"
        ...
        for (( i=1; i<=$ITERATIONS; i++ )); do
            echo -e "${BLUE}>>> Starting $CURRENT_TYPE Migration (${CURRENT_SIZE}MB) - Iteration $i${NC}"
            ...
            echo -e "${GREEN}>>> $CURRENT_TYPE Migration (${CURRENT_SIZE}MB) Iteration $i completed${NC}"
            echo ""
        done
        
        echo -e "${GREEN}=== $CURRENT_TYPE Migration Tests (${CURRENT_SIZE}MB) Completed ===${NC}"
        echo ""
    done
...
if [ "$MIGRATION_TYPE" = "all" ]; then
    ...
    echo "$MIGRATION_TYPE migration completed for all sizes."
fi

...
get_migration_details() {
    ...
    # For precopy and hybrid, wait longer as they will have more iterations due to dirty pages
    ...
    MAX_RETRIES=40  # Increased for precopy/hybrid migrations
    RETRY=0
    while [[ $MIGRATION != *"completed"* && $RETRY -lt $MAX_RETRIES ]]; do
        sleep 15  # Increased check interval
...
# Starting the VM in Source Machine - FIXED
start_source() {
...
for MIGRATION_TYPE_RUN in "${MIGRATION_TYPES[@]}"; do
    echo "=== Starting $MIGRATION_TYPE_RUN Migration Tests ==="
    ...
    for (( i=1; i<=$ITERATIONS; i++ )); do
        echo ">>> Starting $CURRENT_TYPE Migration - Iteration $i"
        ...
        echo ">>> $CURRENT_TYPE Migration Iteration $i completed"
        echo ""
    done
    
    echo "=== $CURRENT_TYPE Migration Tests Completed ==="
    echo ""
done

echo ">>> Migration testing completed for all types"
if [ "$TYPE" = "all" ]; then

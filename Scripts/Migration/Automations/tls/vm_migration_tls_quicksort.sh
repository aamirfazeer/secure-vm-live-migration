...
# Start workload - FINAL CORRECT VERSION FOR MIGRATION
start_workload() {
    echo ">>> Starting Quicksort Workload (detached inside VM)"
    ...
    while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
        if sshpass -p "$VM_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 root@$VM_IP \
            "echo 'VM Ready'" > /dev/null 2>&1; then
            echo ">>> VM is ready"
            VM_READY=true
            break
        fi
        ...
        echo ">>> Waiting for VM... (${WAIT_COUNT}s / ${MAX_WAIT}s)"
    done

    if [ "$VM_READY" = false ]; then
        echo ">>> ERROR: VM not accessible after ${MAX_WAIT}s!"
        return 1
    fi

    # Ensure binary exists
    echo ">>> Checking quicksort binary..."
    if ! sshpass -p "$VM_PASS" ssh -o StrictHostKeyChecking=no root@$VM_IP \
        "test -x /home/vmuser/Desktop/quicksort"; then
        
        echo ">>> ERROR: quicksort binary not found!"
        return 1
    fi

    echo ">>> Launching quicksort in detached mode as vmuser..."
    sshpass -p "$VM_PASS" ssh -o StrictHostKeyChecking=no root@$VM_IP bash -s <<'EOF'
cd /home/vmuser/Desktop || exit 1
chmod +x ./quicksort 2>/dev/null || true
su - vmuser -s /bin/bash -c "setsid nohup /home/vmuser/Desktop/quicksort > /home/vmuser/quicksort.log 2>&1 < /dev/null & echo \$! > /home/vmuser/quicksort.pid"
EOF

    sleep 5

    if check_workload_status; then
        echo ">>> Quicksort launched successfully"
        return 0
    else
        echo ">>> ERROR: Quicksort failed to start!"
        return 1
    fi
}
...
collect_quicksort_output() {
    echo ">>> Collecting quicksort output from VM..."

    sshpass -p "$VM_PASS" ssh -o StrictHostKeyChecking=no root@$VM_IP \
        "cat /home/vmuser/quicksort.log 2>/dev/null" \
        > "logs/${LOG_FOLDER}/${LOG_ID}_quick_sort.txt" 2>&1

    local size=$(wc -c < "logs/${LOG_FOLDER}/${LOG_ID}_quick_sort.txt" 2>/dev/null || echo 0)
    echo ">>> Collected ${size} bytes of quicksort output"
}
...
for (( i=1; i<=$ITERATIONS; i++ )); do
   ...
   echo ">>> Waiting 60 seconds so you can visually verify migration in VNC..."
for sec in {1..60}; do
    printf "\r>>> %d seconds remaining..." $((60-sec))
    sleep 1
done
echo
    terminate-qemu
